...
ulong waybar::modules::IdleInhibitor::deactivationTime = time(nullptr);
...
waybar::modules::IdleInhibitor::IdleInhibitor(const std::string& id, const Bar& bar,
  ...
  event_box_.signal_scroll_event().connect(
      sigc::mem_fun(*this, &IdleInhibitor::handleScroll));
...
auto waybar::modules::IdleInhibitor::update() -> void {
  ...
  label_.set_markup(fmt::format(fmt::runtime(format_),
                                ...
                                fmt::arg("timeout", timeout),
                                fmt::arg("timeleft", timeleft),
  ...
  if (status && timeout) {
    ...
    timeout_ = Glib::signal_timeout().connect_seconds(
        []() {
          ...
          int timeleft = (deactivationTime - time(nullptr)) / 60;
          spdlog::info("updating timeleft. deactivation timestamp: {}, minutes left: {}", deactivationTime, timeleft);
          if (timeleft <= 0) {
